...
char* apply_flags(Arguments* args, char* buffer, size_t* len) {
  ...
  if (args->t_flag) {
    return_value = two_sym_replacement(buffer, len, t_flag_validator, '^',
                                       t_flag_second_value);
    prepare_for_fill(&buffer, &return_value);
  }
  if (args->b_flag) {
  ...
  if (args->e_flag) {
    return_value = two_sym_replacement(buffer, len, e_flag_validator, '$',
                                       e_flag_second_value);
    prepare_for_fill(&buffer, &return_value);
  }
  if (args->v_flag) {
    return_value = two_sym_replacement(buffer, len, v_flag_validator, '^',
                                       v_flag_second_value);
    prepare_for_fill(&buffer, &return_value);
  }
  if (args->n_flag) {
...
char* two_sym_replacement(char* src, size_t* len, int (*validator)(char),
  ...
  char* modified = (char*)calloc(sizeof(char), new_len + 1);
  for (size_t src_index = 0, res_index = 0; src_index < *len;
       ++src_index, ++res_index) {
...
void insert_counter_beginning(char* src, const char* format, size_t ind,
                              int num) {
